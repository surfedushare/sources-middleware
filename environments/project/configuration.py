...
Any configuration that you want to override can be set by using environment variables prefixed with "INVOKE_".
For instance: if you want to override the django.debug configuration for acceptance set INVOKE_DJANGO_DEBUG=0.
If you leave empty any INVOKE environment variables they are assumed to be unset. Use "0" for a False value.
"""
import os
import json
...
for key, value in os.environ.items():
    if key.startswith("INVOKE_") and value == "":
...
def create_configuration_and_session(use_aws_default_profile=True):
    """
